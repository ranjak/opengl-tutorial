...
namespace Framework
{
	...
	bool Timer::Update()
	{
    ogl::seconds timeSecs = System::now();
    float absCurrTime = timeSecs.count();
		if(!m_hasUpdated)
		{
	...
	float Timer::GetAlpha() const
	{
		...
		{
		case TT_LOOP:
      return std::fmod(m_secAccumTime, m_secDuration) / m_secDuration;
		case TT_SINGLE:
	...
	float Timer::GetProgression() const
	{
		...
		{
		case TT_LOOP:
      return std::fmod(m_secAccumTime, m_secDuration);
		case TT_SINGLE:
