...
ProgramData ScaleLighting::LoadProgram(const std::string &strVertexShader, const std::string &strFragmentShader)
{
  ProgramData data;
...
ScaleLighting::ScaleLighting(Window* window) :
  Tutorial(window),
  ...
  mDrawScaledCyl(false),
  mUseInverseTransp(true),
  ...
  mObjtPole(g_initialObjectData, 90.0f/250.0f, glutil::MB_RIGHT_BTN, &mViewPole)
{
  ...
  {
    ScaleLighting* thisTut = static_cast<ScaleLighting*>(win->getTutorial());
...
void ScaleLighting::framebufferSizeChanged(int w, int h)
{
...
void ScaleLighting::renderInternal()
{
...

  {
    ...
    //Render the Cylinder
    {
      ...
      if (mDrawScaledCyl) {
        modelMatrix.Scale(1.0f, 1.0f, 2.0f);
      }
...
void ScaleLighting::MouseMotion(int x, int y)
{
  Framework::ForwardMouseMotion(mViewPole, x, y);
  Framework::ForwardMouseMotion(mObjtPole, x, y);
}

void ScaleLighting::MouseButton(int button, int state, int mods)
{
...
void ScaleLighting::onKeyboard(int key, Window::Action act)
{
  ...
  switch (key) {
  case 32:
    mDrawScaledCyl = !mDrawScaledCyl;
    break;
  case 'T':
    mUseInverseTransp = !mUseInverseTransp;
    break;
  default:
    break;
  }
