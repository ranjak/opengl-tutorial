#include "blinnphonglighting.hpp"
#include "shader.hpp"
...
BlinnPhongLighting::BlinnPhongLighting(Window* window) :
  Tutorial(window),
  ...
  mDiffuseColor(1.0f)
{
  ...
  {
    BlinnPhongLighting* thisTut = static_cast<BlinnPhongLighting*>(win->getTutorial());
...
ProgramData BlinnPhongLighting::LoadProgram(const std::string &strVertexShader, const std::string &strFragmentShader)
{
  ProgramData data;
...
UnlitProgramData BlinnPhongLighting::LoadUnlitProgram(const std::string &strVertexShader, const std::string &strFragmentShader)
{
...
glm::vec4 BlinnPhongLighting::CalcLightPosition()
{
...
void BlinnPhongLighting::framebufferSizeChanged(int w, int h)
{
...
void BlinnPhongLighting::MouseMotion(int x, int y)
{
  Framework::ForwardMouseMotion(mViewPole, x, y);
  Framework::ForwardMouseMotion(mObjtPole, x, y);
}

void BlinnPhongLighting::MouseButton(int button, int state, int mods)
{
...
void BlinnPhongLighting::MouseWheel(int offset)
{
  Framework::ForwardMouseWheel(mViewPole, mWindow, offset);
  Framework::ForwardMouseWheel(mObjtPole, mWindow, offset);
}

void BlinnPhongLighting::onKeyboard(int key, Window::Action act, int mods)
{
