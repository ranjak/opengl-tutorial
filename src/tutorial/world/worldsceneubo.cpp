...
WorldSceneUBO::WorldSceneUBO(Window* window) :
  Tutorial(window),
  mGlobalMatricesUBO(0),
  mGlobalMatricesBindingIndex(0),
  mUniformColor(LoadProgram("shaders/world/PosOnlyWorldTransformUBO.vert", "shaders/world/ColorUniform.frag", mGlobalMatricesBindingIndex)),
  mObjectColor(LoadProgram("shaders/world/PosColorWorldTransformUBO.vert", "shaders/world/ColorPassthrough.frag", mGlobalMatricesBindingIndex)),
  mUniformColorTint(LoadProgram("shaders/world/PosColorWorldTransformUBO.vert", "shaders/world/ColorMultUniform.frag", mGlobalMatricesBindingIndex)),
  ...
  mSphereCamRelPos(67.5f, -46.0f, 150.0f)
{
  ...
  glBindBufferRange(GL_UNIFORM_BUFFER, mGlobalMatricesBindingIndex, mGlobalMatricesUBO, 0, sizeof(glm::mat4) * 2);
...
void WorldSceneUBO::onKeyPressed(Window* win, int key, int, Window::KeyAction act, short mods)
{
  ...
  WorldSceneUBO* thisScene = static_cast<WorldSceneUBO*>(win->getTutorial());
...
void WorldSceneUBO::DrawTree(glutil::MatrixStack& modelMatrix, float fTrunkHeight, float fConeHeight)
{
  //Draw trunk.
  {
...
void WorldSceneUBO::DrawColumn(glutil::MatrixStack& modelMatrix, float fHeight)
{
...
void WorldSceneUBO::DrawParthenon(glutil::MatrixStack& modelMatrix)
{
  //Draw base.
  {
    glutil::PushStack push(modelMatrix);
...
void WorldSceneUBO::DrawForest(glutil::MatrixStack& modelMatrix)
{
...
glm::vec3 WorldSceneUBO::ResolveCamPosition()
{
...
void WorldSceneUBO::renderInternal()
{
  ...
  if(mDrawLookatPoint)
  {
    ...
    modelMatrix.Translate(mCamTarget);
...
void WorldSceneUBO::framebufferSizeChanged(int w, int h)
{
